...
def generate_calificaciones_excel(materia_id, datos_calificaciones, periodo_nombre="PRIMAVERA 2026", docente_nombre="M.C. LUIS YAEL MÉNDEZ SÁNCHEZ"):
    ...
    # Aplicar fondo azul y bordes a cabecera
    for r in range(1, 5):
    ...
    for p in ponderaciones:
        ...
        pct = p.get("porcentaje", 0.0)
        p_name = p.get("nombre_categoria", "Ponderación").upper()
        
        col_span = len(acts) + 1
        start_letter = get_column_letter(current_col)
        end_letter = get_column_letter(current_col + col_span - 1)
        
        # Combinar Fila 5 para el Nombre de Categoría y Porcentaje
        ws_calif.merge_cells(f"{start_letter}5:{end_letter}5")
        cell_p = ws_calif[f"{start_letter}5"]
        cell_p.value = f"{p_name} ({pct}%)"
        cell_p.font = bold_font
        cell_p.alignment = Alignment(horizontal="center", vertical="center")
        cell_p.fill = fill_blue_light
        
        # Escribir nombres de actividades y total en fila 6 y 7
        for act in acts:
            act_name = act.get("nombre", "Actividad").upper()
            
            ws_calif.merge_cells(start_row=6, start_column=current_col, end_row=7, end_column=current_col)
            cell_act = ws_calif.cell(row=6, column=current_col, value=act_name)
            cell_act.font = Font(name="Calibri", size=9, bold=True)
            cell_act.alignment = Alignment(text_rotation=90, horizontal="center", vertical="center")
            current_col += 1
            
        ws_calif.merge_cells(start_row=6, start_column=current_col, end_row=7, end_column=current_col)
        cell_sub = ws_calif.cell(row=6, column=current_col, value="TOTAL")
        cell_sub.font = bold_font
        cell_sub.alignment = Alignment(horizontal="center", vertical="center")
        cell_sub.fill = fill_yellow
        current_col += 1
        
    # Columna FINAL en encabezados
    ws_calif.cell(row=5, column=last_col_idx, value="CALIF.").font = bold_font
    ws_calif.cell(row=5, column=last_col_idx).alignment = Alignment(horizontal="center", vertical="center")
    ws_calif.cell(row=6, column=last_col_idx, value="FINAL").font = bold_font
    ws_calif.cell(row=6, column=last_col_idx).alignment = Alignment(horizontal="center", vertical="center")
    ws_calif.cell(row=7, column=last_col_idx, value="(0-10)").font = bold_font
    ws_calif.cell(row=7, column=last_col_idx).alignment = Alignment(horizontal="center", vertical="center")
    
    # Bordes en filas 5 a 7
    for r in range(5, 8):
        for c in range(1, last_col_idx + 1):
            ws_calif.cell(row=r, column=c).border = thin_border
            if r == 5 and c >= 3 and c < last_col_idx:
                ws_calif.cell(row=r, column=c).fill = fill_blue_light
                
    ws_calif.row_dimensions[5].height = 20
    ws_calif.row_dimensions[6].height = 45
    ws_calif.row_dimensions[7].height = 20
    
    # 3. Escribir Datos de Calificaciones (Fila 8 en adelante)
    ...
    for al in alumnos:
        ...
        ws_calif.cell(row=row_idx, column=2, value=nombre).alignment = Alignment(horizontal="left")
...
def generate_asistencias_excel(materia_id, datos_calificaciones, datos_asistencias, periodo_nombre="PRIMAVERA 2026", docente_nombre="M.C. LUIS YAEL MÉNDEZ SÁNCHEZ"):
    wb = openpyxl.Workbook()
    ws_asist = wb.active
